...
class PUM_API_Reviews {
	...
	public static function insert_record( $values = array() ) {
		...
		// Insert record.
		$wpdb->insert( $wpdb->pum_reviews, $values, array( '%s', '%s', '%s', '%s' ) );
	...
	public static function install() {
		...
		$version         = 2;
		$current_version = get_option( '_pum_review_tracking_table_ver', 0 );

		$wpdb->pum_reviews = $wpdb->prefix . "pum_reviews";

		if ( $current_version < $version ) {
			$charset_collate = $wpdb->get_charset_collate();

			$sql = "CREATE TABLE $wpdb->pum_reviews (
			...
			update_option( '_pum_review_tracking_table_ver', $version );
		}
	}

	public static function template_redirect() {
		if ( ! isset( $_REQUEST['pum-reviews'] ) ) {
			return;
		}
...
add_action( 'plugins_loaded', array( 'PUM_API_Reviews', 'init' ) );
